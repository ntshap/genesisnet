...
class IdentityService {
  ...
  async loadIdentity(storedIdentity) {
    try {
      ...
      } else if (identityData.publicKey && identityData.privateKey) {
        // Legacy format - direct keys
        const publicKey = hexToSeed(identityData.publicKey);
        const privateKey = hexToSeed(identityData.privateKey);
        // Create identity from existing keys
        this.identity = await generateIdentity(publicKey, privateKey);
      } else {
  ...
  async createNewIdentity() {
    try {
      // Generate a random identity using our custom implementation
      this.identity = await generateIdentity();
      ...
      const identityData = {
        type: 'seed',
        seed: seedToHex(this.identity.privateKey),
        publicKey: seedToHex(this.identity.publicKey),
