...
// Import browser-compatible implementations
 ...
 */
export function icpToE8sBigInt(amount) {
 ...
 */
export function e8sToIcpNumber(e8s) {
  ...
  static createActor(idlFactory, { agent, canisterId }) {
    ...
    const actor = {
      ...
      async account_balance({ account }) {
        console.log('Checking balance for account:', accountIdToHex(account));
...
/**
 * Create an ICP ledger interface
 * @param {Identity} identity - The identity to use for authentication
 * @returns {object} The ledger interface
 */
export async function createLedgerActor(identity) {
  try {
    ...
    // Only needed when developing against a local replica
...
class ICPLedgerService {
  ...
  async initialize(identity) {
    try {
      this.identity = identity;
      this.ledgerActor = await createLedgerActor(identity);
